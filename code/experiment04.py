...
def model(params):
    ...
    with tf.variable_scope('symbol'):
        with tf.variable_scope('cnn'):
            ...
            features_symbol = tf.reshape(features, tf.stack([tf.cast(feature_width, 'int32'), input_shape[0],
        ...
        with tf.variable_scope('rnn'):
            placeholders['symbol'] = rnn_block(features_symbol, placeholders, params, 0)
    ...
    with tf.variable_scope('position'):
        with tf.variable_scope('cnn'):
            ...
            feature_dim = params['conv_filter_n'][-1] * (params['img_height'] / height_reduction)
            feature_width = input_shape[2] // width_reduction
            features_position = tf.reshape(features, tf.stack([tf.cast(feature_width, 'int32'), input_shape[0],
                                                    tf.cast(feature_dim, 'int32')]))  # -> [width, batch, features]
        ...
        with tf.variable_scope('rnn'):
            placeholders['position'] = rnn_block(features_position, placeholders, params, 1)
    
    with tf.variable_scope('joint'):
        with tf.variable_scope('cnn'):
            # Convolutional outputs
            features_joint = tf.concat((features_symbol, features_position), 2)

        with tf.variable_scope('rnn'):
            placeholders['joint'] = rnn_block(features_joint, placeholders, params, 2)
