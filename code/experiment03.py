...
def model(params):
    ...
    with tf.variable_scope('symbol'):
        with tf.variable_scope('cnn'):
            ...
            x_symbol, width_reduction, height_reduction = cnn_block(input, params)

            # Prepare output of conv block for recurrent blocks
            features = tf.transpose(x_symbol, perm=[2, 0, 3, 1])  # -> [width, batch, height, channels] (time_major=True)
    ...
    with tf.variable_scope('position'):
        with tf.variable_scope('cnn'):
            # Convolutional blocks
            x_position, width_reduction, height_reduction = cnn_block(input, params)

            x_position = tf.concat((x_symbol, x_position), 3)

            # Prepare output of conv block for recurrent blocks
            features = tf.transpose(x_position, perm=[2, 0, 3, 1])  # -> [width, batch, height, channels] (time_major=True)
            feature_dim = 2 * params['conv_filter_n'][-1] * (params['img_height'] / height_reduction)
            feature_width = input_shape[2] // width_reduction
            features = tf.reshape(features, tf.stack([tf.cast(feature_width, 'int32'), input_shape[0],
                                                    tf.cast(feature_dim, 'int32')]))  # -> [width, batch, features]
